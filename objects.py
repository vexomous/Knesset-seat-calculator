...
class SharingAgreement():
    ...
    def calc_individual_standards(self):
        self.party1_standard = (self.party1.votes) / (self.party1.seats + self.party1_bonus_seats + 1)
        if self.party2 != None:
            self.party2_standard = (self.party2.votes) / (self.party2.seats + self.party2_bonus_seats + 1)
        else:
            self.party2_standard = 0
        
    def calc_standard(self):
        if self.party2 == None:
            self.standard = self.party1.votes / (self.party1.seats + self.bonus_seats + 1)
        else:
            self.standard = (self.party1.votes + self.party2.votes) / (self.party1.seats + self.party2.seats + self.bonus_seats + 1)
